...
import { SearchableSelect, SearchableMultiSelect, SelectOption } from "@/components/ui/SearchableSelect";
...
import { ApiError, AssessmentSuiteOut, createTraining, fetchAssessmentSuites, ModuleConfig } from "@/api/training";
...
const TRAINING_HUB_OPTIONS: SelectOption[] = ["Delhi", "Not Assigned", "BOLPUR", "ALIPURDUAR", "BONGAIGAON", "BAHARAMPUR"].map(
  (v) => ({ label: v, value: v })
);
const AUDIENCE_OPTIONS: SelectOption[] = ["PC Training", "SEC Plan", "SEC", "SEC LITE GT", "OT SEC", "FESTIVE SEC", "SGC"].map(
  (v) => ({ label: v, value: v })
);
...
const TRAINING_TYPE_OPTIONS: SelectOption[] = ["Product Training", "Classroom Training", "Webinar"].map((v) => ({
  label: v,
  value: v,
}));
...
const DEFAULT_CATEGORY_OPTIONS: SelectOption[] = ["POST TEST", "SAMSUMG S25", "Survey", "Quiz"].map((v) => ({
  label: v,
  value: v,
}));
  ...
  const validateModuleSequence = (): string | null => {
    ...
    (["standardTest", "liveQuiz", "survey"] as ModuleKey[]).forEach((key) => {
      if (modules[key].enabled) {
        sequence.push({ label: MODULE_LABELS[key], start: modules[key].startTime ?? "", end: modules[key].endTime ?? "" });
      }
    });
      ...
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <AppCard style={styles.card}>
            ...
            <View style={styles.row}>
              <View style={styles.half}>
                <AppText style={styles.fieldLabel} weight={FontWeight.medium}>Zone</AppText>
                <AppSelect
                  selectedValue={zone}
                  onValueChange={(v) => {
                    setZone(v);
                  ...
                  items={[{ label: "Select Zone", value: "" }, ...ZONES.map((z) => ({ label: z, value: z }))]}
                />
              </View>
              <View style={styles.half}>
                <AppText style={styles.fieldLabel} weight={FontWeight.medium}>Region</AppText>
                <AppSelect
                  selectedValue={region}
                  onValueChange={setRegion}
                  items={[
                    { label: zone ? "Select Region" : "Select Zone First", value: "" },
                    ...(REGIONS_BY_ZONE[zone] ?? []).map((r) => ({ label: r, value: r })),
                  ]}
                />
              </View>
            </View>
            <View style={styles.row}>
              <View style={styles.half}>
                <AppInput label="Company" placeholder="Company name" value={company} onChangeText={setCompany} />
              </View>
              <View style={styles.half}>
                <SearchableSelect
                  label="Requested By"
            ...
            {requestedByOption === "Other" && (
              <AppInput placeholder="Enter Name" value={requestedByOther} onChangeText={setRequestedByOther} />
            )}
          </AppCard>
          ...
          <AppCard style={styles.card}>
            ...
            <View style={styles.row}>
              <View style={styles.half}>
                <AppText style={styles.fieldLabel} weight={FontWeight.medium}>State</AppText>
                <AppSelect
                  ...
                  items={[{ label: "Select State", value: "" }, ...STATES.map((s) => ({ label: s.label, value: s.value }))]}
                />
              </View>
              <View style={styles.half}>
                <AppText style={styles.fieldLabel} weight={FontWeight.medium}>District</AppText>
                <AppSelect
                  selectedValue={district}
                  onValueChange={setDistrict}
                  items={[{ label: "Select District", value: "" }, ...(selectedState?.cities ?? [])]}
                />
              </View>
            </View>
            <AppInput label="Venue" placeholder="Venue address" value={venue} onChangeText={setVenue} />
          </AppCard>

          <AppCard style={styles.card}>
            <SectionTitle index={3} title="Training Details" />
            <Pressable style={styles.toggleRow} onPress={() => setIsResidential((v) => !v)}>
              <AppText style={styles.toggleLabel}>Is this a Residential Program?</AppText>
              <Switch value={isResidential} onValueChange={setIsResidential} trackColor={{ true: Colors.mainColour1 }} />
            </Pressable>

            <View style={styles.row}>
              <View style={styles.half}>
                <DateTimeField label="Training Date *" value={conferenceDate} mode="date" onChange={setConferenceDate} />
              </View>
              <View style={styles.half}>
                <DateTimeField label="Start Time *" value={conferenceTime} mode="time" onChange={setConferenceTime} />
              </View>
            </View>
            <SearchableSelect
              label="Training Hub"
          ...
          <AppCard style={styles.card}>
            ...
            <View style={styles.chipRow}>
              ...
              >
                <Ionicons name="people" size={14} color={attendanceEnabled ? Colors.white : Colors.gray600} />
                <AppText style={styles.chipLabel} color={attendanceEnabled ? Colors.white : Colors.gray600}>Attendance</AppText>
              </Pressable>
              {(Object.keys(MODULE_LABELS) as ModuleKey[]).map((key) => (
                <Pressable
                  key={key}
                  style={[styles.chip, modules[key].enabled && styles.chipActive]}
                  onPress={() => toggleModule(key)}
                >
                  <Ionicons name={MODULE_ICONS[key]} size={14} color={modules[key].enabled ? Colors.white : Colors.gray600} />
                  <AppText style={styles.chipLabel} color={modules[key].enabled ? Colors.white : Colors.gray600}>
                    {MODULE_LABELS[key]}
                  </AppText>
                </Pressable>
              ))}
            ...
            {!attendanceEnabled && !Object.values(modules).some((m) => m.enabled) && (
            ...
            {attendanceEnabled && (
              <View style={styles.moduleCard}>
                <View style={styles.moduleHeader}>
                  ...
                  <AppText style={styles.moduleTitle} weight={FontWeight.semiBold}>ATTENDANCE</AppText>
                  <Pressable onPress={() => setAttendanceEnabled(false)} hitSlop={8}>
                    <Ionicons name="trash-outline" size={18} color={Colors.danger} />
                  </Pressable>
                </View>
                <View style={styles.row}>
                  <View style={styles.half}>
                    <DateTimeField label="Check-In Opens" value={checkInOpens} mode="time" onChange={setCheckInOpens} />
                  </View>
                  <View style={styles.half}>
                    <DateTimeField label="Check-Out Closes" value={checkOutCloses} mode="time" onChange={setCheckOutCloses} />
                  </View>
                </View>
                <Pressable style={styles.checkboxRow} onPress={() => setGeoFencing((v) => !v)}>
                  <View style={[styles.checkbox, geoFencing && styles.checkboxChecked]}>
                    {geoFencing && <Ionicons name="checkmark" size={12} color={Colors.white} />}
                  </View>
            ...
            {(Object.keys(MODULE_LABELS) as ModuleKey[])
              ...
              .map((key) => (
                <View key={key} style={styles.moduleCard}>
                  <View style={styles.moduleHeader}>
                    <Ionicons name={MODULE_ICONS[key]} size={16} color={Colors.mainColour1} />
                    <AppText style={styles.moduleTitle} weight={FontWeight.semiBold}>{MODULE_LABELS[key].toUpperCase()}</AppText>
                  ...
                  <View style={styles.row}>
                    <View style={styles.half}>
                      <SearchableSelect
                        ...
                        onSelect={(option) =>
                          updateModule(key, { category: option.value, assessmentSuiteUid: undefined, questionCount: "" })
                        }
                      />
                    </View>
                    <View style={styles.half}>
                      <SearchableSelect
                        ...
                        placeholder={modules[key].category ? "Select Test" : "Select a category first"}
                        value={modules[key].assessmentSuiteUid ?? ""}
                        options={questionSetOptionsFor(modules[key].category)}
                        onSelect={(option) => {
                          const suite = assessmentSuites.find((s) => s.assessmentSuiteUid === option.value);
                          updateModule(key, {
                  ...
                  <View style={styles.row}>
                    <View style={styles.half}>
                        ...
                        {modules[key].assessmentSuiteUid && (
                          <View style={styles.maxBadge}>
                            <AppText style={styles.maxBadgeText} color={Colors.white}>
                              Max: {assessmentSuites.find((s) => s.assessmentSuiteUid === modules[key].assessmentSuiteUid)?.noOfQuestion ?? 0}
                            </AppText>
                          </View>
                        )}
                      </View>
                      <AppInput
                        ...
                        onChangeText={(text) => updateModule(key, { questionCount: text })}
                      />
                    </View>
                  </View>
                  <View style={styles.row}>
                  ...
                  <View style={styles.row}>
                    <Pressable
                      ...
                      onPress={() => updateModule(key, { checkIn: !modules[key].checkIn })}
                    >
                      <View style={[styles.checkbox, modules[key].checkIn && styles.checkboxChecked]}>
                        {modules[key].checkIn && <Ionicons name="checkmark" size={12} color={Colors.white} />}
                      </View>
                    ...
                    <View style={styles.half}>
                      ...
                      <AppSelect
                        selectedValue={modules[key].unlockCondition ?? "Automatic"}
                        onValueChange={(v) => updateModule(key, { unlockCondition: v })}
                        items={UNLOCK_CONDITIONS.map((u) => ({ label: u, value: u }))}
                      />
                    </View>
                  </View>
                </View>
          ...
          <AppCard style={styles.card}>
            ...
            <Pressable style={styles.checkboxRow} onPress={() => setAgreeTerms((v) => !v)}>
              ...
              <AppText style={styles.checkboxLabel}>I agree to the Terms &amp; Conditions</AppText>
            </Pressable>

            {notice && <AppText style={styles.notice}>{notice}</AppText>}

            <AppButton title="Register Training Session" onPress={handleSubmit} loading={submitting} />
          </AppCard>
        </ScrollView>
      </SafeAreaView>
    </>
...
const styles = StyleSheet.create({
  ...
  emptyFlow: { alignItems: "center", gap: 8, paddingVertical: 24, backgroundColor: Colors.gray100, borderRadius: Radius.xl },
