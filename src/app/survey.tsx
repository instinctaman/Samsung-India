...
export default function SurveyScreen() {
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Couldn't load the survey.");
    } finally {
      setLoading(false);
    }
  }, [token, suiteUid]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Couldn't submit the survey.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (submittedAt) {
    const attempted = Object.keys(answers).filter((key) => answers[Number(key)]?.trim()).length;
    ...
    return (
      <TestSubmittedView
        ...
        rows={[
          { label: "Survey Title", value: surveyTitle ?? "Feedback Survey", icon: "document-text", iconColor: Colors.success, iconBg: "#D8F8EB" },
          {
            label: "Date & Time",
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
      <ScreenBanner backgroundColor={Colors.mainColour1}>
        <AppText style={styles.bannerTitle} color={Colors.white} weight={FontWeight.semiBold} numberOfLines={2}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {questions.map((question, index) => (
          <View key={question.id} style={styles.questionCard}>
            <AppText style={styles.questionTag} color={Colors.mainColour1}>Question {index + 1} of {questions.length}</AppText>
            ...
            {FREE_TEXT_TYPES.has(question.question_type) ? (
              ...
              <View style={styles.options}>
                {question.options.map((option) => {
                  ...
                  return (
                    <Pressable key={option.id} style={[styles.option, selected && styles.optionSelected]} onPress={() => setAnswer(index, option.id)}>
        ...
        <Pressable style={[styles.submitButton, submitting && styles.disabledButton]} disabled={submitting} onPress={handleSubmit}>
          {submitting ? <ActivityIndicator color={Colors.white} /> : <AppText color={Colors.white} weight={FontWeight.semiBold}>Continue</AppText>}
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  retryButton: { backgroundColor: Colors.mainColour1, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8 },

  bannerTitle: { fontSize: Fonts.h3, lineHeight: 22 },

  content: { padding: 14, gap: 12, paddingBottom: 28 },
  questionCard: {
    borderRadius: 10, padding: 12, backgroundColor: Colors.white,
    shadowColor: Colors.black, shadowOpacity: 0.06, shadowOffset: { width: 0, height: 2 }, shadowRadius: 6, elevation: 2,
  },
  questionTag: { alignSelf: "flex-start", fontSize: Fonts.overline, backgroundColor: "#DDEEFF", paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4 },
  question: { marginTop: 8, fontSize: Fonts.bodySm, lineHeight: 19 },

  options: { marginTop: 10, gap: 8 },
  option: {
    minHeight: 44, borderWidth: 1, borderColor: Colors.gray200, borderRadius: 8, paddingHorizontal: 12,
    flexDirection: "row", alignItems: "center", gap: 10,
  },
  optionSelected: { borderColor: Colors.mainColour1, backgroundColor: "#EEF4FF" },
  radio: { width: 18, height: 18, borderRadius: 9, borderWidth: 2, borderColor: Colors.gray400, alignItems: "center", justifyContent: "center" },
  radioSelected: { borderColor: Colors.mainColour1 },
  radioDot: { width: 9, height: 9, borderRadius: 5, backgroundColor: Colors.mainColour1 },
  optionText: { flex: 1, fontSize: Fonts.bodySm },

  textInput: {
    marginTop: 10, minHeight: 84, borderWidth: 1, borderColor: Colors.gray200, borderRadius: 8,
    padding: 12, fontSize: Fonts.bodySm, color: Colors.black, textAlignVertical: "top",
  },

  inlineError: { color: Colors.danger, fontSize: Fonts.bodySm, textAlign: "center" },

  submitButton: { height: 48, borderRadius: Radius.xl, backgroundColor: Colors.mainColour1, alignItems: "center", justifyContent: "center", marginTop: 4 },
