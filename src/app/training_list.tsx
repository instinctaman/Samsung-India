...
const CONFERENCE_STATUS_PRESENTATION: Record<string, { label: string; color: string; background: string }> = {
...
const APPROVAL_STATUS_PRESENTATION: Record<string, { label: string; color: string; background: string; icon: keyof typeof Ionicons.glyphMap }> = {
  Pending: { label: "PENDING", color: "#B45309", background: "#FEF3E2", icon: "hourglass-outline" },
  Approved: { label: "APPROVED", color: Colors.success, background: "#DCFCE7", icon: "checkmark-circle-outline" },
  Rejected: { label: "REJECTED", color: Colors.danger, background: "#FCE4E4", icon: "close-circle-outline" },
};
    ...
    <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
      <ScreenBanner backgroundColor={Colors.mainColour1}>
        <View style={styles.bannerRow}>
          ...
          <View>
            <AppText style={styles.bannerTitle} color={Colors.white} weight={FontWeight.semiBold}>Training List</AppText>
            <AppText style={styles.bannerSubtitle} color={Colors.white}>All your sessions, with or without approval</AppText>
          </View>
        </View>
      </ScreenBanner>
      ...
      >
        {loading ? (
          ...
          <View style={styles.centered}>
            ...
            <AppText style={styles.emptyTitle} weight={FontWeight.medium}>No Trainings Yet</AppText>
...
function TrainingCard({ item, onPress }: { item: TrainingAgendaItem; onPress: () => void }) {
  const conferencePresentation = CONFERENCE_STATUS_PRESENTATION[item.conferenceStatus] ?? {
    label: item.conferenceStatus,
    color: Colors.gray600,
    background: Colors.gray100,
  };
  const approvalPresentation = APPROVAL_STATUS_PRESENTATION[item.approvalStatus] ?? {
  ...
  return (
    <Pressable style={[styles.card, { borderColor: conferencePresentation.color }]} onPress={onPress}>
      <View style={styles.cardHeaderRow}>
        <View style={styles.chipRow}>
          <View style={[styles.statusChip, { backgroundColor: conferencePresentation.background }]}>
            <View style={[styles.statusDot, { backgroundColor: conferencePresentation.color }]} />
            <AppText style={[styles.statusChipText, { color: conferencePresentation.color }]} weight={FontWeight.bold}>
              {conferencePresentation.label}
            </AppText>
          </View>
          <View style={[styles.statusChip, { backgroundColor: approvalPresentation.background }]}>
            <Ionicons name={approvalPresentation.icon} size={11} color={approvalPresentation.color} />
            <AppText style={[styles.statusChipText, { color: approvalPresentation.color }]} weight={FontWeight.bold}>
