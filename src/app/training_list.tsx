...
import {  fetchTrainerAgenda } from "@/api/training";
...
// Preserved alternate implementation from the upstream/Tushar merge (card-based UI).
// TrainingListScreen above is the active route (renders via TrainingListView) — this
// version is kept intact rather than deleted since the same card/hook logic is relied on
// elsewhere and shouldn't be lost.
function TrainingListCardView() {
  const router = useRouter();
  const { items, loading, refreshing, refresh } = useTrainerAgendaList(false);
